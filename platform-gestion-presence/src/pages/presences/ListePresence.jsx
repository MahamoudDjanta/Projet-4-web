...
import { closePresenceSession, createPresenceSession, markManualPresence } from '../../api/presences'
...
export default function ListePresence() {
  ...
  async function openSession(event) {
    ...
    const { data: response } = await createPresenceSession({ cours_id: form.cours_id })
  ...
  async function closeSession() {
    if (form.mode === 'manuel') {
      await markManualPresence(session.id, {
        session_id: session.id,
        presences: students.map((student) => ({
          eleve_id: student.id,
          statut: selected[student.id] ? 'present' : 'absent',
        })),
      })
    }
    ...
    <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
      <Card>
        <form className="grid gap-4" onSubmit={openSession}>
          <label className="grid gap-2 text-sm font-medium text-[var(--text)]">
            Classe
            <select className="h-11 rounded-lg border border-gray-200 px-3 focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500" value={form.classe_id} onChange={(event) => setForm({ ...form, classe_id: event.target.value, cours_id: '' })} required>
      ...
      <div className="grid gap-4">
        <Card className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button variant="danger" disabled={!session} onClick={closeSession}><Check className="h-4 w-4" /> Clôturer la liste</Button>
        </Card>
        {!session ? (
          <EmptyState title="Aucune liste ouverte" description="Sélectionnez une classe et un cours pour commencer." />
        ) : form.mode === 'qr' ? (
          <SessionQrCode value={session.qr_token || session.local_token} title={session.cours?.nom || 'Session de présence'} subtitle={session.opened_at} />
        ) : (
