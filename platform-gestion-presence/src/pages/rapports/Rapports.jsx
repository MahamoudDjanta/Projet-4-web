    ...
    <div className="grid gap-6">
      <Card>
        <form className="grid gap-4 md:grid-cols-4" onSubmit={preview}>
          ...
          <Input label="Date de début" type="date" value={filters.date_debut} onChange={(event) => setFilters({ ...filters, date_debut: event.target.value })} required />
          <Input label="Date de fin" type="date" value={filters.date_fin} onChange={(event) => setFilters({ ...filters, date_fin: event.target.value })} required />
          <Input label="Cours" value={filters.cours_id} onChange={(event) => setFilters({ ...filters, cours_id: event.target.value })} />
          <div className="flex gap-2 md:col-span-4">
            <Button type="submit"><Search className="h-4 w-4" /> Aperçu</Button>
            <Button variant="secondary" onClick={download} disabled={!report}><Download className="h-4 w-4" /> Télécharger PDF</Button>
          </div>
        </form>
      </Card>
